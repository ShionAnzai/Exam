...
public class TestListSubjectDao extends Dao {
    ...
    public List<TestListSubject> filter(int entYear, String classNum, Subject subject, School school)
        ...
        ResultSet rSet = null;
        // SQL文の雛形テーブル
        String from = "(select test.student_no, test.subject_cd, test.school_cd, test.no, test.point, test.class_num from test join student on test.student_no = student.no where student.ent_year = ? and subject_cd = ? and test.class_num = ? and test.school_cd = ? and test.no = 1 order by test.student_no) as a";
        // SQL文の結合
        ...
        try {
            ...
            statement = connection.prepareStatement(baseSql + " from " + from + join + condition + join2 + order);
